...
class ResPartner(models.Model):
    ...
    def cron_archive_customers_no_invoices(self):
        ...
        one_month_ago = fields.Date.to_date(fields.Date.today() - timedelta(days=30))
        ...
        for partner in customers:
            ...
            # 🔹 Count posted invoices in last month
        ...
        # 🔹 Prepare email body
        message = _(
            "Automatic Archive: %s customers were archived (no invoices for 1 month)."
        ) % archived_count

        if skipped_due_to_users:
            names = "<br/>• " + "<br/>• ".join(skipped_due_to_users.mapped("name"))
            message += _(
                "<br/><br/>⚠ The following customers were skipped because they are linked to active users:<br/>%s"
            ) % names
        ...
        if admin and admin.email:
            ...
            mail = Mail.create({
                'subject': _("Customer Auto-Archive Report"),
                'body_html': f"<p>{message}</p>",
                'email_to': admin.email,
                'email_from': self.env.company.email or admin.email,  # fallback
            })
            mail.send()
